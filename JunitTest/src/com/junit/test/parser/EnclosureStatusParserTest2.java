...
public class EnclosureStatusParserTest2 {
	...
	public void enclosureStatusTest() throws Exception {
		...
		String enclosureStatusCommandResult = " enclosure   500056b3a784f4ff\n" + 
				" management  this enclosure is managed via the default interface\n" + 
				" model       DP BP14G+EXP with 16 bays (2 used)\n" + 
				" capacity    3.20TB on 2 disk(s) (2 SSD)\n" + 
				" pools       rpool\n" + 
		...
		String[] rrSplit = enclosureStatusCommandResult.split("\r\r");
		
		int rrIndex = 0;
		for (String rrStr : rrSplit) {		
			System.out.println("[EnclosureStatusParser] rrStr: [" + rrIndex + "] "+ rrStr);
			rrIndex++;
		}
		
		System.out.println("========================================================================");
		
		String[] rnSplit = enclosureStatusCommandResult.split("\r\n");
		
		int rnIndex = 0;
		for (String rnStr : rnSplit) {		
			System.out.println("[EnclosureStatusParser] rnStr: [" + rnIndex + "] "+ rnStr);
			rnIndex++;
		}
		
		System.out.println("========================================================================");
		
		String[] rnrnSplit = enclosureStatusCommandResult.split("\r\n\r\n");
		
		int rnrnIndex = 0;
		for (String rnrnStr : rnrnSplit) {		
			System.out.println("[EnclosureStatusParser] rnrnStr: [" + rnrnIndex + "] "+ rnrnStr);
			rnrnIndex++;
		}
		...
		String[] sepSplit = enclosureStatusCommandResult.split(System.lineSeparator());
		
		int sepIndex = 0;
		for (String sepStr : sepSplit) {		
			System.out.println("[EnclosureStatusParser] sepStr: [" + sepIndex + "] "+ sepStr);
			sepIndex++;
		}
		...
		String[] sepSepSplit = enclosureStatusCommandResult.split(System.lineSeparator()+System.lineSeparator());
		
		int sepSepIndex = 0;
		for (String sepSepStr : sepSepSplit) {		
			System.out.println("[EnclosureStatusParser] sepSepStr: [" + sepSepIndex + "] "+ sepSepStr);
			sepSepIndex++;
		}
		
		System.out.println("========================================================================");
		
		String[] lineSplit = enclosureStatusCommandResult.split("\n");
		
		int lineIndex = 0;
		for (String lineStr : lineSplit) {
			System.out.println("[EnclosureStatusParser] lineStr: [" + lineIndex + "]" + lineStr);
			lineIndex++;
		}
